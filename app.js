(function () {
  ...
  const deckSessionOptions = [
    { id: "quick", title: "Quick", cards: 5, xp: 20, reason: "Quick-Runde geschafft" },
    { id: "normal", title: "Normal", cards: 10, xp: 40, reason: "Deck abgeschlossen" },
    { id: "focus", title: "Focus", cards: 20, xp: 90, reason: "Fokus-Runde geschafft" }
  ];
  ...
  const dueCards = () => allCardsForReview().filter(cardIsDue);
  ...
  const awardCardReviewXp = (card, wasDue) => {
    ...
    if (previous?.date === today) return 0;
    const xp = wasDue ? 4 : 1;
    state.cardXpHistory[card.id] = { date: today, xp, due: Boolean(wasDue) };
    if (xp > 0) {
  ...
  const deckSessionOption = (id) => deckSessionOptions.find((option) => option.id === id) || deckSessionOptions[1];
  const deckSessionCountToday = (deckId) => {
    const session = state.deckSessions?.[deckId];
    return session?.date === todayIso() ? Number(session.count || 0) : 0;
  };
  const deckSessionRewardInfo = (deckId, option) => {
    const repeatCount = deckSessionCountToday(deckId);
    if (repeatCount > 0) {
      return { xp: 10, reason: "Übungsbonus", label: "Heute schon gelernt: nur noch +10 XP Übungsbonus verfügbar", sameDeck: true, reduced: true };
    }
    const afterDailyLimit = Number(state.dailyDeckStats?.fullSessions || 0) >= 3;
    const xp = afterDailyLimit ? Math.max(1, Math.round(Number(option.xp || 0) * 0.5)) : Number(option.xp || 0);
    return {
      xp,
      reason: option.reason,
      label: afterDailyLimit ? `${xp} XP verfügbar, weil heute schon 3 volle Runden geschafft sind` : `Dieses Deck: ${xp} XP verfügbar`,
      sameDeck: false,
      reduced: afterDailyLimit
    };
  };
  const cardsForSessionOption = (option) => {
    const cards = baseStudyCards();
    const due = cards.filter(cardIsDue);
    if (due.length > 0 && due.length <= option.cards) return due;
    if (due.length > option.cards) return due.slice(0, option.cards);
    return cards.slice(0, option.cards);
  };
  const resetCardStudySession = () => {
    state.cardStudySession = defaultCardStudySession();
    state.ui.cardStudyIndex = 0;
  };
  const startDeckSession = (optionId) => {
    const option = deckSessionOption(optionId);
    const meta = currentDeckMeta();
    const cards = cardsForSessionOption(option);
  ...
  const completeDeckSession = () => {
    ...
    const option = deckSessionOption(session.optionId);
    const reward = deckSessionRewardInfo(session.deckId, option);
    ...
    if (reward.xp > 0) {
      awardXp(reward.xp, reward.reason);
      xpEarned += reward.xp;
      deckXp += reward.xp;
      state.dailyDeckStats.deckXp = Number(state.dailyDeckStats.deckXp || 0) + reward.xp;
    }
    const allDueFinished = session.dueCardIds?.length > 0 && session.dueCardIds.every((id) => reviewedIds.includes(id));
    if (!reward.sameDeck && allDueFinished) {
      awardXp(15, "Alle fälligen Karten geschafft");
      xpEarned += 15;
      bonusXp += 15;
    }
    if (!reward.sameDeck && session.perfect !== false && reviewedIds.length > 0) {
      awardXp(15, "Perfekte Runde");
      xpEarned += 15;
      bonusXp += 15;
    }
    const challengeXp = reward.sameDeck ? 0 : completeWeeklyDeckChallenge();
    ...
    if (!reward.sameDeck) state.dailyDeckStats.fullSessions = Number(state.dailyDeckStats.fullSessions || 0) + 1;
    ...
    const summary = {
      cardsReviewed: reviewedIds.length,
      xpEarned,
      deckXp,
      ...
      nextReviewDate,
      allDueFinished,
      perfect: session.perfect !== false,
      repeatedDeck: reward.sameDeck
    };
  ...
  const rateDeckSessionCard = (card, rating) => {
    ...
    const mistakesCreated = Number(session.mistakesCreated || 0) + (rating === "again" || rating === "hard" ? 1 : 0);
    state.cardStudySession = {
      ...state.cardStudySession,
      ...session,
      reviewedIds,
    ...
    return `
      ...
      <section class="deck-session-choice card-study-view">
        ...
        <article class="session-progress-card">
          ...
          <div>
            <span class="eyebrow">Stapel bereit</span>
            <h2>${C.escapeHtml(meta.title)}</h2>
            <p>${dueInDeck ? `${dueInDeck} Karten sind heute fällig.` : `${deckCards.length} Karten können freiwillig wiederholt werden.`}</p>
          </div>
        </article>
        <div class="deck-option-grid">
          ${deckSessionOptions.map((option) => {
            const cards = cardsForSessionOption(option);
            const reward = deckSessionRewardInfo(meta.id, option);
            return `
              <button class="deck-option-card start-deck-session" data-option="${option.id}" type="button" ${cards.length ? "" : "disabled"} aria-label="${option.title} Runde starten, ${cards.length} Karten, ${reward.xp} XP">
                <span>${C.escapeHtml(option.title)}</span>
                <strong>${cards.length} Karten</strong>
        ...
        <p class="panel-note">XP gibt es erst, wenn du Karten bewertest. Umdrehen allein zählt nicht.</p>
      </section>
    `;
  };
  const renderDeckSessionSummary = () => {
    ...
    const progress = levelProgress();
    return `
      ${C.sectionTitle("Karten", "Runde abgeschlossen")}
      <section class="session-summary-card card-study-view">
        <div class="study-view-header">
          <button class="secondary-button close-study-view" type="button">Zurück</button>
          <strong>${C.escapeHtml(session.deckTitle || "Stapel")}</strong>
        </div>
        <article class="session-progress-card celebration">
          ${C.mascot("mascot-small")}
          <div>
            <span class="eyebrow">Starke Runde</span>
            <h2>Du hast ${Number(summary.xpEarned || 0)} XP verdient.</h2>
            <p>${summary.repeatedDeck ? "Wiederholung zählt als Übungsbonus." : "Dein Deck-Fortschritt wurde gespeichert."}</p>
          </div>
        </article>
        <div class="session-summary-grid">
          <article><span>Karten</span><strong>${Number(summary.cardsReviewed || 0)}</strong></article>
          <article><span>Karten-XP</span><strong>${Number(summary.cardXp || 0)}</strong></article>
          <article><span>Combo-Bonus</span><strong>${Number(summary.comboXp || 0)}</strong></article>
        ...
        <div class="continue-actions">
          <button class="primary-button start-more-cards" data-option="quick" type="button">Noch 5 Karten lernen</button>
          <button class="secondary-button choose-other-deck" type="button">Anderes Deck starten</button>
    ...
    return `
      ...
      <section class="flashcard-study-area card-study-view">
        ...
        <article class="session-mini-status">
          <span>Runde</span><strong>${reviewed} / ${Number(session.target || cards.length)}</strong>
          <span>Combo</span><strong>${Number(session.combo || 0)}</strong>
  ...
  const renderCards = () => {
    ...
    const challenge = state.weeklyDeckChallenge || defaultWeeklyDeckChallenge();
    return `
      ...
      <section class="card-progress-strip" aria-label="Karten-Fortschritt">
        <article><span>Heute fällig</span><strong>${dueCount}</strong></article>
        <article><span>Runden heute</span><strong>${Number(state.dailyDeckStats?.sessionsCompleted || 0)}</strong></article>
        <article><span>Karten-XP heute</span><strong>${Number(state.dailyDeckStats?.cardXp || 0)}</strong></article>
        <article class="wide"><span>Wochenchallenge</span><strong>${Number(challenge.progress || 0)} / ${Number(challenge.target || 5)}</strong>${progressBar(challenge.progress, challenge.target, "Wochenchallenge Deck-Runden", "compact")}</article>
      </section>
  ...
  const bindEvents = (route) => {
    ...
    if (route === "cards") {
      ...
      document.querySelectorAll(".start-deck-session").forEach((button) => button.addEventListener("click", () => {
        startDeckSession(button.dataset.option);
        save();
        render();
      }));
      document.querySelector(".start-more-cards")?.addEventListener("click", (event) => {
        state.cardStudySession = { ...defaultCardStudySession(), choosing: true };
        startDeckSession(event.currentTarget.dataset.option || "quick");
        save();
        render();
      });
